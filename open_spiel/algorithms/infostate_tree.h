...
class NodeId {
  ...
  public:
   NodeId(size_t id_value, const InfostateTree* tree_ptr) : identifier_(id_value), tree_(tree_ptr)
   {
   }
...
MakeInfostateTree(const Game& game, Player acting_player, int max_move_limit = 1000);
...
class InfostateTree final {
  ...
  private:
   ...
   // Friend factories.
   friend std::shared_ptr< InfostateTree > MakeInfostateTree(const Game&, Player, int);
   ...
   friend std::shared_ptr< InfostateTree >
   MakeInfostateTree(const std::vector< const InfostateNode* >&, int);
  ...
  public:
   ...
   const InfostateNode* observation_infostate(const SequenceId& sequence_id) const
   {
      SPIEL_DCHECK_TRUE(sequence_id.BelongsToTree(this));
      return sequences_.at(sequence_id.id());
   }
   ...
   DecisionId DecisionIdFromInfostateString(const std::string& infostate_string) const;

   // -- Leaf operations --------------------------------------------------------
   const std::vector< InfostateNode* >& leaf_nodes() const { return nodes_at_depths_.back(); }
   ...
   std::pair< double, SfStrategy > BestResponse(TreeplexVector< double > gradient) const;
   ...
   // -- For debugging ----------------------------------------------------------
   // the previous version would have needed to use this operator in the following way:
   // tree << std::cout; for an infostate tree object
   // But normal usage would be std::cout << tree.
   // --> should be a friend operator
  ...
  private:
   ...
   void BuildTerminalNode(
      InfostateNode* parent,
      size_t depth,
      const State& state,
      double chance_reach_prob
   );
   void BuildDecisionNode(
      InfostateNode* parent,
      size_t depth,
      const State& state,
      int move_limit,
   ...
   CollectStartEndSequenceIds(InfostateNode* node, const SequenceId parent_sequence);
};
...
class VecWithUniquePtrsIterator {
  ...
  public:
   explicit VecWithUniquePtrsIterator(const std::vector< std::unique_ptr< T > >& vec, int pos = 0)
       : pos_(pos), vec_(vec)
   {
   }
   ...
   bool operator==(VecWithUniquePtrsIterator other) const { return pos_ == other.pos_; }
   bool operator!=(VecWithUniquePtrsIterator other) const { return ! (*this == other); }
   T* operator*() { return vec_[pos_].get(); }
   VecWithUniquePtrsIterator begin() const { return *this; }
   VecWithUniquePtrsIterator end() const { return VecWithUniquePtrsIterator(vec_, vec_.size()); }
};
...
class InfostateNode final {
  ...
  public:
   ...
   {
      return infostate_string_ != kFillerInfostate && infostate_string_ != kDummyRootNodeInfostate;
   }
   ...
   Range< SequenceId > AllSequenceIds() const
   {
      return Range< SequenceId >(start_sequence_id_.id(), end_sequence_id_.id(), &tree_);
   }
   ...
   size_t corresponding_states_size() const { return corresponding_states_.size(); }
...
class TreeVector {
  ...
  public:
   ...
   TreeVector(const InfostateTree* tree, std::vector< T > vec) : tree_(tree), vec_(std::move(vec))
   {
